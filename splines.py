...
def cubic_spline_3pts(x,y,T):
    """
    Apperently scipy.inpterpolate.interp1d does not support 
    ...
    k = np.array(np.linalg.inv(M)*v)   #线性代数基础范数向量或规范矩阵的逆###线性代数基础范数向量或矩阵范数inv方形矩阵求逆求解线性方程组det方形矩阵的行列式slogdet方阵的行列式lstsq求解线性最小二乘问题
    ...
    t2 = (T[np.r_[T>=x1]&np.r_[T<=2]]-x1)/x2x1
    t11, t22 = 1.-t1, 1.-t2
